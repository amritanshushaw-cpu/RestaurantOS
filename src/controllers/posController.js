...
document.addEventListener('DOMContentLoaded', () => {
    ...
    html += paymentSessions.map(s => {
      return `
        <div style="background: var(--color-primary); border: 1px solid #8b5cf6; padding: 12px; border-radius: 8px; min-width: 250px; border-left: 4px solid #8b5cf6;">
          ...
          <button type="button" class="btn-sentry btn-waiter-payment" data-session="${s.session_id}" data-type="${s.payment_type}" aria-label="Verify and Accept Payment for ${s.table_no}" style="width: 100%; background: #8b5cf6; color: #fff; padding: 10px 14px; min-height: 44px; font-weight: 700; font-size: 13px; border-radius: 6px; cursor: pointer; border: none;">
  ...
  function openWaiterBillPaymentModal(initialTable) {
    ...
    const btnConfirmPay = document.getElementById('btn-confirm-pos-payment');
    const btnResetTbl = document.getElementById('btn-reset-pos-table-orders');

    btnConfirmPay?.addEventListener('click', () => {
      if (btnConfirmPay.disabled) return;

      btnConfirmPay.disabled = true;
      if (btnResetTbl) btnResetTbl.disabled = true;

      let countdown = 5;
      btnConfirmPay.style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
      btnConfirmPay.style.color = '#ffffff';
      btnConfirmPay.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> Awaiting Payment Confirmation... (${countdown}s)`;
      ...
      const timer = setInterval(() => {
        ...
        if (countdown > 0) {
          ...
          // Payment Confirmation Completed (5s elapsed)
          btnConfirmPay.style.background = 'linear-gradient(135deg, #10b981, #059669)';
          btnConfirmPay.style.color = '#ffffff';
          btnConfirmPay.innerHTML = `<i class="fa-solid fa-circle-check"></i> Payment Successful, thanks for visiting!`;
          ...
          try {
            ...
            if (window.authService && window.authService.showToast) {
              window.authService.showToast(`🎉 Payment Successful for ${stdTable}! Thanks for visiting!`);
            }
