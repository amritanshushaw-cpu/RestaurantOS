...
class EntryGatewayModal {
  ...
  handleSelectRole(role, targetPage) {
    ...
    // Open Real Authentication window (Google OAuth / Email OTP / Quick Enter) for target role
  ...
  renderModal(forceShow = false) {
    ...
    const modalHtml = `
      <div id="entry-gateway-modal" class="entry-gateway-backdrop" style="display:flex;">
        <div class="entry-modal-card" style="max-width: 1040px; width: 94%;">
          ...
          <div class="entry-modal-footer" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
            <button type="button" id="btn-modal-google-oauth" class="btn-google-sign-in" style="padding: 8px 16px; font-size: 12px;">
            ...
            <span style="font-size:12px; color:var(--text-tertiary);">RestaurantOS v3.1 · Supabase Real Auth Enabled</span>
          </div>
        </div>
      </div>
    `;
    ...
    const btnGoogleOAuth = document.getElementById('btn-modal-google-oauth');
    ...
    btnGoogleOAuth?.addEventListener('click', () => authService.loginWithGoogle('Customer'));
