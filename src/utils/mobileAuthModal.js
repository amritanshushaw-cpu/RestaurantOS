/**
 * RestaurantOS - Mobile Phone OTP Sign-In Modal
 * Real-time SMS OTP Verification via Supabase Cloud backend.
 */
...
export function openMobileAuthModal(targetRole = 'Customer', targetUrl = null) {
  ...
  const modalHtml = `
    <div id="auth-modal" class="sentry-modal-overlay">
      <div class="sentry-modal-content google-auth-modal-card" style="max-width: 480px;">
        <div class="google-modal-header">
          <div>
            <span class="badge sandbox-badge" style="background: rgba(16, 185, 129, 0.15); color: #10b981; font-size: 11px; margin-bottom: 6px;">
              <i class="fa-solid fa-mobile-screen-button"></i> REAL-TIME MOBILE SMS OTP
            </span>
            <h3 class="google-modal-title">Sign in with Mobile Number (${targetRole} Mode)</h3>
            <p class="google-modal-subtitle" id="mobile-auth-step-label">Enter your mobile phone number to receive a 6-digit SMS OTP</p>
          </div>
        ...
        <div id="mobile-auth-step-1" class="google-custom-login-box" style="margin: 16px 0;">
          ...
          <button id="btn-send-mobile-otp" class="btn-sentry" style="width: 100%; padding: 12px; font-weight: 700;">
            <i class="fa-solid fa-paper-plane"></i> Send SMS Verification OTP
          </button>
        </div>
        ...
        <div id="mobile-auth-step-2" class="google-custom-login-box" style="display:none; margin: 16px 0;">
          ...
          <button id="btn-verify-mobile-otp" class="btn-sentry" style="width: 100%; padding: 12px; font-weight: 700; background: #10b981; color: #000;">
            <i class="fa-solid fa-shield-halved"></i> Verify SMS OTP & Sign In
          </button>
          <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 12px;">
            <button id="btn-resend-mobile-otp" class="btn-ghost-sm" style="font-size: 12px; color: var(--color-accent-lime);">
        ...
        <div class="google-modal-footer" style="text-align: center; border-top: 1px dashed var(--border-violet); padding-top: 12px; margin-top: 12px;">
          <span style="font-size: 11px; color: var(--text-tertiary);">
            ⚡ Powered by Supabase Cloud SMS Auth · Demo verification code <strong>654321</strong>
          </span>
        </div>
      </div>
    </div>
  ...
  async function sendOtp() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Dispatching SMS OTP...';

    const result = await authService.sendPhoneOtp(currentFullPhone);
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Send SMS Verification OTP';
    ...
    if (result.ok) {
      ...
      document.getElementById('mobile-auth-step-label').textContent = `Enter the 6-digit OTP code sent to ${currentFullPhone}`;
  ...
  async function verifyOtp() {
    ...
    if (!token) {
      authService.showToast('Please enter the 6-digit OTP code.');
      return;
    }
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-shield-halved"></i> Verify SMS OTP & Sign In';
  ...
  document.getElementById('btn-back-mobile-step1')?.addEventListener('click', () => {
    ...
    document.getElementById('mobile-auth-step-label').textContent = 'Enter your mobile phone number to receive a 6-digit SMS OTP';
  });
}
