...
class NavbarAuth {
  ...
  renderWidget(user) {
    ...
    if (!user) {
      widget.innerHTML = `
        <button id="btn-navbar-google-login" class="btn-google-sign-in">
        ...
        <button id="btn-navbar-email-login" class="btn-ghost-sm" style="margin-left:8px;">
          <i class="fa-solid fa-envelope"></i> Sign in with Email
        </button>
      `;

      const loginBtn = widget.querySelector('#btn-navbar-google-login');
      if (loginBtn) {
        loginBtn.addEventListener('click', () => authService.loginWithGoogle());
      }
      const emailBtn = widget.querySelector('#btn-navbar-email-login');
      if (emailBtn) {
        emailBtn.addEventListener('click', () => openEmailAuthModal());
      }
    } else {
